...
def ramp(curr_angle, prev_angle, base_speed):
    diff = curr_angle - prev_angle
    if abs(diff) < 0.5:
        return base_speed
    adjustment = (diff/6) * 20
    new_s = base_speed + adjustment
    return clamp(new_s, MIN_BASE_SPEED, MAX_SPEED)
...
try:
    while running and not btn.any():
        ...
        # GYRO STUFF
        try: 
            gyro_now = float(gyro.value())
            change_speed = ramp(gyro_now, prev_angle, change_speed)
            prev_angle = gyro_now
        except Exception:
            pass
        ...
        if not neither_on:
            ...
            err = vL - vR      # want this to be 0
            integral += err * dt
            deriv = (err - prev_err) / dt
            u = Kp * err + Ki * integral + Kd * deriv

            left_cmd  = change_speed + u
            right_cmd = change_speed - u
            ...
            if both_on:
                ...
                base_speed_now = int(change_speed * 1.0)
