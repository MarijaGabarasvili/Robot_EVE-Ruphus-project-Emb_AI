...
SEARCH_TIME_LIMIT = 4.0          # maximum search time in seconds
SEARCH_TURN_DURATION = 2.0      # duration to turn in one direction during search

# PID gains
Kp, Ki, Kd = -5.0, 0.0, -1.0
...
try:
    while not btn.any():
        ...
        if neither_on:
            if off_l is None:
                off_l = time.time()      # start timing (losing the line)
                set_speeds(0, 0)        # stop
                search_start_l = time.time()
                search_phase = "left"   # start searching to the left
            elapsed_off = time.time() - off_l

            # 1. If still within time limit, continue searching
            if elapsed_off < SEARCH_TIME_LIMIT: 
                if search_phase == "left":
                    set_speeds(-SEARCH_TURN, +SEARCH_TURN)  # Spin Left
                    if time.time() - search_start_l >= SEARCH_TURN_DURATION:
                        search_phase = "right"
                        search_start_l = time.time()
                ...
                # Right search phase
                    ...
                    # Keeps turning right until time limit reached

            # 3. The timer runs out -> stop and exit
            else:
